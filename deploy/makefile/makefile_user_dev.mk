...
DOCKER_FILE = ../dockerfile/dockerfile_$(SERVER_NAME)_$(SERVER_TYPE)_dev

# 检查并删除目标文件和目标镜像
clean:
	@echo "Checking if ${SERVER_NAME}-${SERVER_TYPE} exists..."
	@if [ -f ../dockerfile/bin/${SERVER_NAME}-${SERVER_TYPE} ]; then \
		echo "Deleting ${SERVER_NAME}-${SERVER_TYPE}..."; \
		rm -f ../dockerfile/bin/${SERVER_NAME}-${SERVER_TYPE}; \
	fi
...
build:
	...
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o ../dockerfile/bin/${SERVER_NAME}-${SERVER_TYPE} ../../apps/${SERVER_NAME}/${SERVER_TYPE}/${SERVER_NAME}.go
